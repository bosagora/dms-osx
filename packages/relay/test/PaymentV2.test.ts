...
describe("Test of Server", function () {
    ...
    const paymentSigner = Wallet.createRandom();
    ...
    context("Test point relay endpoints", () => {
        ...
        context("Test of Loyalty Point", () => {
            ...
            it("Endpoint POST /v2/payment/info", async () => {
            ...
            it("Endpoint POST /v2/payment/new/open", async () => {
                const url = URI(serverURL).directory("/v2/payment/new").filename("open").toString();
                ...
                params.signature = await ContractUtils.signMessage(paymentSigner, message);
            ...
            it("Endpoint POST /v2/payment/item", async () => {
                const url = URI(serverURL).directory("/v2/payment/item").addQuery("paymentId", paymentId).toString();
                const response = await client.get(url);
            ...
            it("Endpoint POST /v2/payment/new/approval - deny", async () => {
                const responseItem = await client.get(
                    URI(serverURL).directory("/v2/payment/item").addQuery("paymentId", paymentId).toString()
                );
                ...
                const response = await client.post(
                    URI(serverURL).directory("/v2/payment/new").filename("approval").toString(),
                    {
                        paymentId,
                        approval: false,
                        signature,
            ...
            it("Endpoint POST /v2/payment/new/approval", async () => {
                const responseItem = await client.get(
                    URI(serverURL).directory("/v2/payment/item").addQuery("paymentId", paymentId).toString()
                );
                const nonce = await ledgerContract.nonceOf(users[purchaseOfLoyalty.userIndex].address);
            ...
            it("Endpoint POST /v2/payment/new/approval - wrong ID", async () => {
                const responseItem = await client.get(
                    URI(serverURL).directory("/v2/payment/item").addQuery("paymentId", paymentId).toString()
                );
                const nonce = await ledgerContract.nonceOf(users[purchaseOfLoyalty.userIndex].address);
            ...
            it("Endpoint POST /v2/payment/new/approval - Wrong signature", async () => {
                const responseItem = await client.get(
                    URI(serverURL).directory("/v2/payment/item").addQuery("paymentId", paymentId).toString()
                );
                const nonce = await ledgerContract.nonceOf(users[purchaseOfLoyalty.userIndex].address);
            ...
            it("Endpoint POST /v2/payment/new/close", async () => {
                const params = {
                    paymentId,
                    confirm: true,
                    signature: "",
                };
                const message = ContractUtils.getCloseNewPaymentMessage(params.paymentId, params.confirm);
                params.signature = await ContractUtils.signMessage(paymentSigner, message);
                const response = await client.post(
                    URI(serverURL).directory("/v2/payment/new").filename("close").toString(),
                    params
                );
    ...
    context("Test point relay endpoints - Cancel Confirm", () => {
        ...
        context("Test of Loyalty Point", () => {
            ...
            it("Endpoint POST /v2/payment/cancel/open", async () => {
                const url = URI(serverURL).directory("/v2/payment/cancel").filename("open").toString();
                const params = {
                    paymentId,
                    terminalId: "TM000001",
                    signature: "",
                };
                const message = ContractUtils.getOpenCancelPaymentMessage(params.paymentId, params.terminalId);
                params.signature = await ContractUtils.signMessage(paymentSigner, message);
                const response = await client.post(url, params);
            ...
            it("Endpoint POST /v2/payment/cancel/approval - true", async () => {
                const responseItem = await client.get(
                    URI(serverURL).directory("/v2/payment/item").addQuery("paymentId", paymentId).toString()
                );
                ...
                const url = URI(serverURL).directory("/v2/payment/cancel").filename("approval").toString();
                const params = {
                    paymentId,
                    approval: true,
                    signature,
            ...
            it("Endpoint POST /v2/payment/cancel/close", async () => {
                const params = {
                    paymentId,
                    confirm: true,
                    signature: "",
                };
                const message = ContractUtils.getCloseCancelPaymentMessage(params.paymentId, params.confirm);
                params.signature = await ContractUtils.signMessage(paymentSigner, message);
                const response = await client.post(
                    URI(serverURL).directory("/v2/payment/cancel").filename("close").toString(),
                    params
                );
    ...
    context("Test point relay endpoints - Cancel Deny", () => {
        ...
        context("Test of Loyalty Point", () => {
            ...
            it("Endpoint POST /v2/payment/cancel/approval - false", async () => {
                const responseItem = await client.get(
                    URI(serverURL).directory("/v2/payment/item").addQuery("paymentId", paymentId).toString()
                );
                const nonce = await ledgerContract.nonceOf(shopData[purchaseOfLoyalty.shopIndex].wallet.address);
        ...
        const mobilePhone: INotificationEventHandler = {
            receive: async (to: string, title: string, body: string, data: any) => {
                ...
                if (data.type === "new") {
                    ...
                    const responseItem = await client.get(
                        URI(serverURL).directory("/v2/payment/item").addQuery("paymentId", receivedPaymentId).toString()
                    );
                    const nonce = await ledgerContract.nonceOf(users[purchaseOfLoyalty.userIndex].address);
                    const signature = await ContractUtils.signLoyaltyNewPayment(
                        users[purchaseOfLoyalty.userIndex],
                    ...
                    const response = await client.post(
                        URI(serverURL).directory("/v2/payment/cancel").filename("approval").toString(),
                        {
                            paymentId: receivedPaymentId,
                            approval: true,
                            signature,
