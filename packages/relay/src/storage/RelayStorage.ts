    ...
    public postStorePurchase(data: IStorePurchaseData): Promise<void> {
        return new Promise<void>(async (resolve, reject) => {
            this.queryForMapper("purchase", "postStorePurchase", {
                ...
                provideToken: data.providePoint.div(GWI_UNIT).toString(),
                provideValue: data.providePoint.div(GWI_UNIT).toString(),
                shopId: data.shopId.toLowerCase(),
                shopCurrency: data.shopCurrency,
                shopProvidedAmount: data.providePoint.div(GWI_UNIT).toString(),
            })
                .then(() => {
                    return resolve();
                })
                    ...
                    resolve(
                        result.rows.map((m) => {
                            return {
                                ...
                                provideToken: BigNumber.from(m.providePoint.toString()).mul(GWI_UNIT),
                                provideValue: BigNumber.from(m.providePoint.toString()).mul(GWI_UNIT),
                                shopId: m.shopId,
                                shopCurrency: m.shopCurrency,
                                shopProvidedAmount: BigNumber.from(m.providePoint.toString()).mul(GWI_UNIT),
                            };
                        })
                    );
                })
