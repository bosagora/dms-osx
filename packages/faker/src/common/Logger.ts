...
export class Logger {
    ...
    public static defaultFileTransport(folderPath: string) {
        ...
        const options = {
            filename: path.resolve(folderPath, "ValidatorBalance.log"),
            handleExceptions: true,
            json: false,
            maxsize: 10485760, // 10MB
            maxFiles: 10,
            colorize: false,
            format: combine(label({ label: "ValidatorBalance" }), timestamp(), logFormat),
        };
    ...
    public static defaultConsoleTransport() {
        ...
        const options = {
            ...
            json: false,
            colorize: false,
            format: combine(label({ label: "Faker" }), timestamp(), logFormat),
        };
